...
page_table_setup:
   ...
   add %g5, %g1, %g3
   st %g2, [%g3]
   !PTE: context=0, index=2, level=2,  ppnr=0x40080000, p_addr=0x8, cacheable=0x0, acc=0x3
   ! *(PAGE_TABLE_BASE + 0x8) = 0x400800e (pte)
   set 0x400800e, %g2
   set 0x8, %g5
   add %g5, %g1, %g3
   st %g2, [%g3]
   ...
   st %g4, [%g3]
   ! g4 stored into [g3] 
   !PTD: context=0, index=60, level=2, child_p_addr=0x100, p_addr=0x3f0
   ! *(PAGE_TABLE_BASE + 0x3f0) = ptd(PAGE_TABLE_BASE + 0x100)
   ! make PTD from 0x100
   set 0x100, %g4
   add %g1, %g4, %g4
   srl %g4, 0x4, %g4
   ...
   add %g5, %g1, %g3
   st %g2, [%g3]
   !PTE: context=0, index=1, level=3,  ppnr=0xfff01000, p_addr=0x104, cacheable=0x1, acc=0x3
   ! *(PAGE_TABLE_BASE + 0x104) = 0xfff018e (pte)
   set 0xfff018e, %g2
   set 0x104, %g5
   add %g5, %g1, %g3
   st %g2, [%g3]
   !PTD: context=0, index=63, level=2, child_p_addr=0x2cc, p_addr=0x3fc
