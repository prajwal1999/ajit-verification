...
page_table_setup:
   ...
   add %g5, %g1, %g3
   st %g2, [%g3]
   !PTE: context=0, index=3, level=2,  ppnr=0x80000, p_addr=0xc, cacheable=0x1, acc=0x3
   ! *(PAGE_TABLE_BASE + 0xc) = 0x808e (pte)
   set 0x808e, %g2
   set 0xc, %g5
   add %g5, %g1, %g3
   st %g2, [%g3]
   !PTE: context=0, index=4, level=2,  ppnr=0x80000, p_addr=0x10, cacheable=0x1, acc=0x3
   ! *(PAGE_TABLE_BASE + 0x10) = 0x808e (pte)
   set 0x808e, %g2
   set 0x10, %g5
   add %g5, %g1, %g3
   st %g2, [%g3]
